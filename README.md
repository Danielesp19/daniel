# ALTURA — catálogo de café de especialidad

Catálogo web para una tostadora de café colombiano de especialidad. Cada
producto lleva su ficha de origen (finca, productor, altura, variedad,
proceso, notas de cata, puntaje SCA), el pedido sale por WhatsApp y el
inventario se administra por chat.
...
- **Catálogo con fichas técnicas.** Los datos duros del café son el diseño, no
  un adorno: región, altura, variedad, proceso y puntaje van en una rejilla
...
- **Tres formas de mostrar una categoría**, configurables desde el panel:
  ...
  vitrina de a uno (se pasa deslizando, pensada para videos).
- **Carrito que va a WhatsApp.** No hay pasarela de pago: se arma el mensaje
  con el pedido y se abre el chat. Antes de abrirlo se revalida el stock
  contra el servidor, porque el catálogo se sirve cacheado.
- **Inventario por chat.** Los administradores le escriben al bot de WhatsApp
  ("¿cuánto queda del mirador?", "llegaron 12 bolsas") y él consulta o ajusta.
...
php artisan migrate --seed     # crea el catálogo de prueba
...
(`admin@altura.co` / `password` — cámbiala antes de exponerlo).
...
señalar. Los datos de la marca —nombre, WhatsApp, ciudad— viven en
`src/lib/marca.ts`.
...
Un administrador le escribe por WhatsApp y Claude interpreta el mensaje,
consulta el catálogo con herramientas y responde. Puede buscar productos, dar
un resumen del inventario, ajustar stock y editar datos de un producto.
